...
- **Tier D is never package-granted** — Tier D is the one tier that removes the operator. A signature proves who wrote a skill, not that it holds autonomous safety authority, so today Tier D is accepted only from skills shipped with the runtime. That is containment of the legacy path, not the final model: under the release-owned safety profile that replaces it, packages do not participate in the decision at all
