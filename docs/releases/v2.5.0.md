...
Minor release, in progress. `v2.5.0-rc.11` is the current candidate.
...
| `v2.5.0-rc.9` | protection preflight, full test matrix, Android payload build | the release disclosure audit refused a paragraph of `docs/DISPATCH_PLAN.md` for containing an English word that contains a private identifier, so no bundle was built or signed |
| `v2.5.0-rc.10` | published | the phone-launch candidate re-cut with the audit fix; the first release to publish signed Android runtime payloads, each verified before publication and reverified from the public origin, and the first carrying the `runtime-telemetry/v2` delivery rules on both producers |
| `v2.5.0-rc.11` | cut | the commissioning revision candidate: a revision is held to every claimed proof leg whatever changed, `commissioning binding-export` returns the binding in force to revise from, the bridge's reads leave the state database as they found it, and a verified revision replaces the staged binding in force without `--force`; cut to prove the revision half of the commissioning ceremony on the bench Pi |

`v2.5.0-rc.1` produced no artifacts and must not be installed. Nothing was
built, signed or published from it.

`v2.5.0-rc.9` produced no artifacts either, and must not be installed. It was
cut as the phone-launch candidate — the first to publish signed Android runtime
payloads, and the first carrying the `runtime-telemetry/v2` delivery rules on
both producers — and it reached the bundle build.

Its cause was again in the audit rather than the release, and again in
substring matching. The audit's wheelhouse half matches a denylisted identifier
on word boundaries, and says why in its own comment: a term inside an unrelated
word discloses nothing. Its document half still matched by substring, so a
paragraph describing what a trigger's tier means — using an ordinary English
word that happens to contain a private identifier — failed it. No document
disclosed anything.

Both halves now match alike, and the rule is tested directly rather than only
through whichever documents happen to exist: a term inside an English word is
not a finding, while the same term standing alone, inside a repository name,
inside a module path, or upper-cased, is.

The Android payload build did run under this tag and passed, including the
rebuild that proves each payload's digest reproduces on the runner image. What
a tag still has not exercised is signing a payload, which is the job after the
one that failed.

`v2.5.0-rc.10` published. Every job ran, including the two that no tag had
reached before: the release key signed each payload under the
`android_runtime_payload` purpose and its own key id, and the published set was
reverified from the public origin. Each payload was then fetched by tag and
verified independently against the registry the runtime ships — all nine
consumer checks, API level 21 and `stripped` read from the bytes, and a refusal
when one is offered for another ABI slot. The published `arm64-v8a` digest is
the one the pull-request runner produced from the same commit, so two runners of
the same operating system agree on the bytes.

It inherits no hardware certification of its own: nothing in it touches the
install path, so the rc.7 tuple certification is what stands.
...
### An attested action no longer claims execution it did not perform

`executed` on an action row now means an executor ran and reported success. It
did not below Tier D: a dispatched action with no registered executor reported
itself executed, so an approved Tier C action whose executor was never
registered sealed a chain row asserting that the named physical action had been
taken while nothing was driven. Tier D was the only tier that reported this
honestly. A missing executor is now a non-execution at every tier, recorded
distinctly from an executor that ran and failed, because the two need different
remedies.

**Rows sealed before this release cannot be corrected, and must not be
reinterpreted.** They are signed and hash-chained, so amending one is neither
possible nor desirable. A `SAFETY_ACTION_EXECUTED` row from an earlier release
carrying `executed: true` is not, on its own, evidence that a physical action
occurred — the deployment's executor registration at the time decides that, and
the row does not record it. Read such rows as attesting that an action was
dispatched and reached the point of execution. Rows sealed from this release
onward carry the stronger meaning.
...
- WhatsApp business-template delivery is implemented for four closed intents.
  The exact v2/v3 templates are approved, retained real-sender runs reached
  `delivered` or `read` on a handset, and a live Tier C reply was bound to its
  inbound provider message before a non-energising executor ran. Persistent
  deployment of the approved Content SIDs and deliberate WhatsApp-to-SMS
  failover without duplication remain unproven.
- Health-gated rollback is proven end to end on the bench Pi, first against a
  development-signed build of this candidate's preparation commit and then
  against the published `v2.5.0-rc.7` bundle installed with the Pi wheelhouse
  (`docs/releases/evidence/`), which has also been through the runbook's
  clean-host install, reboot and uninstall phases as published. The failing
  candidate is development-signed by design.
- Africa's Talking submission now retains the provider `messageId`, and an
  authenticated delivery report reconciles that ID into a later handset or
  terminal-failure observation. This remains host-tested only until a live SMS,
  deployed signing bridge, provider callback and handset receipt are retained
  together.
