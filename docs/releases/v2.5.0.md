...
- WhatsApp business-template delivery is implemented and host-tested for four
  closed intents, with provider acceptance separated from later handset
  delivery. It is not production-proven: the four exact templates still need
  provider approval, and a real sender/handset run must retain the provider
  message identifier plus a later `delivered` or `read` observation. Sandbox
  free-form acceptance does not close that boundary.
