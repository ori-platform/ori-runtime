...
| `v2.5.0-rc.9` | cut | the phone-launch candidate: the first to publish signed Android runtime payloads, and the first carrying the `runtime-telemetry/v2` delivery rules on both producers. It exists to exercise what only a tag can reach: signing a payload under its own purpose and key id through the release signing authority, the NDK install on the release runner, and a draft whose asset set now includes three payloads beside the bundles |
