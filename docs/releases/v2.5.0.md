...
- WhatsApp alerts cannot be delivered outside a sandbox: the provider models a
  ...
  template.
- Health-gated rollback is proven end to end on the bench Pi against a
  development-signed, generic-wheelhouse build of the `v2.5.0-rc.7`
  preparation commit
  (`docs/releases/evidence/v2.5.0-rc.7-systemd-host-linux-aarch64-python3.13.md`);
  the same transaction has not yet been exercised with a bundle signed through
  the release workflow or with the Raspberry Pi wheelhouse.
