...
| `v2.5.0-rc.7` | published | the evidence exchange candidate re-cut with the installer fix and the shutdown checkpoint fix; installed on the bench Pi against stock `python3.13` through the verified bootstrap with the Pi wheelhouse, survived a reboot, and is the release a deliberately failing upgrade was rolled back onto |
...
- Health-gated rollback is proven end to end on the bench Pi, first against a
  ...
  (`docs/releases/evidence/`). The failing candidate is development-signed by
  design; the runbook's clean-host install and uninstall phases have not been
  repeated with the published bundle.
