...
VECTORS = json.loads(
    (
        Path(__file__).parent.parent / "fixtures" / "firmware_layer1_vectors.json"
    ).read_text()
)
...
FAULT_VECTORS = json.loads(
    (
        Path(__file__).parent.parent / "fixtures" / "firmware_fault_vectors.json"
    ).read_text()
)
...
class TestGoldenFaultVectors:
    ...
    def test_verifier_vocabulary_matches_the_vector_corpus(self) -> None:
        ...
        from ori.security.firmware.telemetry import FIRMWARE_FAULT_CODES
...
class TestFailClosed:
    ...
    def test_closed_vocabularies_match_the_contract_exactly(self) -> None:
        from ori.security.firmware.telemetry import CLOSED_FAULT_DETAILS
