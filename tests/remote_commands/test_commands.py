...
def fixed_now(monkeypatch):
    ...
    monkeypatch.setattr("ori.security.remote_commands.commands.now_ms", lambda: now)
    monkeypatch.setattr("ori.security.remote_commands.throttle.now_ms", lambda: now)
    return now
...
async def test_remote_command_lockout_enforcement_blocks_critical_risk(
    store, fixed_now
):
    from_number = "+2348012345678"
    for idx in range(2):
        await _seed_security_incident(
            ...
            from_number=from_number,
            now=fixed_now,
            incident_id=f"enforced-incident-{idx}",
        )
